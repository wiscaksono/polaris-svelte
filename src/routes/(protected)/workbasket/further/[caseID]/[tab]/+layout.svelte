<script lang="ts">
	...
	const { previousHref, nextHref } = $derived.by(() => {
		...
		return {
			previousHref: previousSlug ? `/workbasket/further/${data.taskFormParams.case_id}/${previousSlug}` : '',
			nextHref: nextSlug ? `/workbasket/further/${data.taskFormParams.case_id}/${nextSlug}` : ''
		};
	});
</script>
...
<ul class="flex gap-2 overflow-x-auto border-b py-2">
	{#each data.currentTaskForm as { title, slug } (title)}
		<a class="text-sm font-semibold whitespace-nowrap" href={`/workbasket/further/${data.taskFormParams.case_id}/${slug}`}>{title}</a>
	{/each}
</ul>
