<script lang="ts">
	...
	const { previousHref, nextHref } = $derived.by(() => {
		...
		return {
			previousHref: previousSlug ? `/workbasket/new-submission/${data.taskFormParams.case_id}/${previousSlug}` : '',
			nextHref: nextSlug ? `/workbasket/new-submission/${data.taskFormParams.case_id}/${nextSlug}` : ''
		};
	});
</script>

<header class="flex justify-between border-b py-2">
	<p>{data.taskFormParams.case_trx}</p>
...
<ul class="flex gap-2 overflow-x-auto border-b py-2">
	{#each data.currentTaskForm as { title, slug } (title)}
		<a class="text-sm font-semibold whitespace-nowrap" href={`/workbasket/new-submission/${data.taskFormParams.case_id}/${slug}`}>{title}</a>
	{/each}
</ul>
