<script lang="ts">
	...
	setTaskFormContext({
		taskFormParams: () => ({
			id: 1,
			reg_spaj: data.taskFormParams.reg_spaj,
			nopol: data.taskFormParams.nopol,
			case_id: data.taskFormParams.case_id,
			nama_user: data.taskFormParams.nama_user,
			no_trx: data.taskFormParams.trx_major,
			no_tmp: data.taskFormParams.no_temp,
			id_doc_tmp: data.taskFormParams.id_doc_temp,
			from_app: data.taskFormParams.from_app,
			nama_prod: data.taskFormParams.nama_prod,
			case_trx: data.taskFormParams.case_trx,
			kind: 'Approval',
			create_at: data.taskFormParams.create_at,
			status_trx: data.taskFormParams.status_trx,
			repricing: 'No'
		}),
