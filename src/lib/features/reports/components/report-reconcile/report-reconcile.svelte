<script lang="ts">
	...
	import { reportReconcileTabs } from './tabs';

	const queryState = useQueryState('tab', parseAsStringLiteral(reportReconcileTabs.map(({ slug }) => slug)).withDefault(reportReconcileTabs[0].slug));
	const currentTab = $derived(reportReconcileTabs.find(({ slug }) => slug === queryState.current));
</script>
...
<nav class="z-30 -mx-4 -mt-3.5 mb-3.5 border-b">
	<ul class="-mb-0.5 flex overflow-x-auto overflow-y-hidden [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
		...
		{#each reportReconcileTabs as { title, slug } (slug)}
