<script lang="ts">
	...
	const query = $derived(createQuery(dataPolisTransactionHistoryQueries.positionSPAJ(regSpaj)));
</script>
	...
	<Table.Root variant="outline">
		<Table.Header class="sticky top-0 z-10 bg-background">
			<Table.Row>
				...
				<Table.Head>Posisi Dokumen</Table.Head>
				<Table.Head>Status Polis</Table.Head>
				<Table.Head>Status Accept</Table.Head>
				<Table.Head>User</Table.Head>
				<Table.Head>Keterangan 1</Table.Head>
				<Table.Head>Keterangan 2</Table.Head>
			</Table.Row>
		</Table.Header>
		<Table.Body>
			{#if $query.isLoading}
			...
			{:else if $query.data}
				{#each $query.data as item, i (i)}
					<Table.Row>
						...
						<Table.Cell>{item.posisi_dokumen}</Table.Cell>
						<Table.Cell>{item.status_polis}</Table.Cell>
						<Table.Cell>{item.status_accept}</Table.Cell>
						<Table.Cell>{item.nama_user}</Table.Cell>
						<Table.Cell>{item.keterangan_1}</Table.Cell>
						<Table.Cell>{item.keterangan_2}</Table.Cell>
					</Table.Row>
				{/each}
			{:else}
				<Table.Row>
