<script lang="ts">
	...
	import { transactionHistoryTabs } from './tabs';

	import type { RouteParams } from '$app/types';

	let props: RouteParams<'/(protected)/data-polis/[regSpaj]/[tab]'> = $props();

	const queryState = useQueryState('tab', parseAsStringLiteral(transactionHistoryTabs.map(({ slug }) => slug)).withDefault(transactionHistoryTabs[0].slug));
	const currentTab = $derived(transactionHistoryTabs.find(({ slug }) => slug === queryState.current));
</script>
...
<nav class="z-30 -mx-4 -mt-3.5 mb-3.5 border-b">
	<ul class="-mb-0.5 flex overflow-x-auto overflow-y-hidden [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
		...
		{#each transactionHistoryTabs as { title, slug } (slug)}
