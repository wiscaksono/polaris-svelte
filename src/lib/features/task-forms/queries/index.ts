...
export const taskFormQueries = {
  transactionHistories: ({ trx_major, case_id, no_temp, id_doc_temp }: { trx_major: string; case_id: number; no_temp: string; id_doc_temp: string }) => {
    return queryOptions({
      ...
      queryFn: async () => {
        const { data } = await api.post<Type.TransactionHistoriesRes>('/polaris/api-business-polaris/major/alteration/viewStatusTrx', { trx_major, case_id, no_temp, id_doc_temp, });
