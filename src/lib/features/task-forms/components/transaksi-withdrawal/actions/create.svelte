<script lang="ts">
	...
	const alreadySelectedFund = $derived(data.transactionData.map((item) => item.fundCode));
...
<Dialog.Root bind:open onOpenChangeComplete={() => (values = initialValues)}>
	...
	<Dialog.Content onEscapeKeydown={handleCloseAttempt} onInteractOutside={handleCloseAttempt}>
		<Dialog.Header>
			<Dialog.Title>Add New Fund</Dialog.Title>
					...
					<Select.Content>
						{#if query.data}
							{#each query.data as { kdFund, nmFund } (kdFund)}
								<Select.Item value={kdFund} disabled={alreadySelectedFund.includes(kdFund)}>{nmFund}</Select.Item>
							{/each}
						{/if}
					</Select.Content>
				</Select.Root>
