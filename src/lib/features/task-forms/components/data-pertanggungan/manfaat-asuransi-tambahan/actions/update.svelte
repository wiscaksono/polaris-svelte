<script lang="ts">
	...
	interface Props {
		data: DataPertanggunganRes['data_pertanggungan']['manfaat_asuransi_tambahan'][number];
		initialData: DataPertanggunganRes;
		index: number;
	}

	let { data, initialData, index }: Props = $props();
...
<Dialog.Root bind:open onOpenChangeComplete={() => (values = data)}>
	...
	<Dialog.Content onEscapeKeydown={handleCloseAttempt} onInteractOutside={handleCloseAttempt} class="max-h-[95svh] overflow-y-auto sm:max-w-3xl">
		<Dialog.Header>
			<Dialog.Title>Update Additional Insurance Benefits</Dialog.Title>
