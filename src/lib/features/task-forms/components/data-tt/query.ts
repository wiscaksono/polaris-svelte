	...
	create: () => {
		return mutationOptions({
			mutationFn: async ({ initialData, newData }: { initialData: DataTTRes; newData: Omit<DataTTRes['tertanggung_tambahan'][number]['after'], 'sar'> }) => {
				const lastNoUrut =
					initialData.tertanggung_tambahan.length > 0 ? parseInt(initialData.tertanggung_tambahan[initialData.tertanggung_tambahan.length - 1].no_urut) : 0;
	...
	update: () => {
		return mutationOptions({
			mutationFn: async ({
				...
				initialData: DataTTRes;
				newData: Omit<DataTTRes['tertanggung_tambahan'][number]['after'], 'sar'>;
				index: number;
			}) => {
				const { data } = await api.post('/polaris/api-business-polaris/major/workbasket/newsub-update-additional-insured', {
					...initialData,
