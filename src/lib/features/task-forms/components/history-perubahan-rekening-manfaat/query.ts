...
export const historyPerubahanRekeningManfaatQueries = {
	get: ({ regSpaj }: { regSpaj: string }) => {
		return queryOptions({
			...
			queryFn: async () => {
				const { data } = await api.get<HistoryPerubahanRekeningManfaatRes>(
					`/polaris/api-financial-polaris/v1/financial/worksheet/getHistoryRekeningManfaat?regSpaj=${regSpaj}`
				);
				return data;
			}
		});
