...
<Dialog.Root bind:open onOpenChangeComplete={() => (values = data)}>
	...
	<Dialog.Content onEscapeKeydown={handleCloseAttempt} onInteractOutside={handleCloseAttempt}>
		<Dialog.Header>
			<Dialog.Title>Update SAR {titleKeyMap[type]}</Dialog.Title>
			<Dialog.Description>Submit the financial analysis for this case. Contact finance if you need help.</Dialog.Description>
		</Dialog.Header>
