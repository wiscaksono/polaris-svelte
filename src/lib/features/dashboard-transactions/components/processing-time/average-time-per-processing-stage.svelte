<script lang="ts">
	...
	let { data }: { data: DashboardTransactionsRes['processingTime']['average'] } = $props();
...
<Card.Root class="flex flex-col rounded-none border-0 border-b bg-transparent shadow-none">
	<Card.Header>
		<Card.Title class="text-primary">Average Time per Processing Stage</Card.Title>
		<Card.Description>View the average duration for each distinct step in the process.</Card.Description>
	</Card.Header>
	<Card.Content>
