...
export const workbasketQueries = {
	...
	furtherList: ({ pageSize = 10, pageNumber = 1 }: { pageSize?: number; pageNumber?: number }) => {
		return queryOptions({
			...
			queryFn: async () => {
				const { data } = await api.post<PolisListRes<'further'>>(
					'/polaris/api-business-polaris/major/workbasket/further',
					{ pageSize, pageNumber }
				);
				return data;
			}
	...
	fillingList: ({ pageSize = 10, pageNumber = 1 }: { pageSize?: number; pageNumber?: number }) => {
		return queryOptions({
			...
			queryFn: async () => {
				const { data } = await api.post<PolisListRes<'filling'>>(
					'/polaris/api-business-polaris/major/workbasket/filling',
					{ pageSize, pageNumber }
				);
				return data;
			}
